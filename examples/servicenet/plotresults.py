...
def print_zipf_results(lst):

    alphas = [0.1, 0.25, 0.50, 0.75, 1.0]
    strategies = ['NDN', 'NDN_MEASUREMENT', 'IPFS', 'OPTIMAL']
    
    # playback interruption counts per strategy
    for strategy in strategies:
        filename = './Results/buffering_events_' + str(strategy)
        f = open(filename, 'w')
        f.write('# Number of playback interruptions.\n')
        f.write('#\n')
        f.write('# Zipf Number_of_Interruptions\n')
        for alpha in alphas:
            buffer_count = searchDictMultipleCat(lst, ['strategy', 'workload'], {'name' : strategy, 'alpha' : alpha}, 2, 'VIDEO_PLAYBACK', 'BUFFER_RATIO')
            s = str(alpha) + "\t" + str(buffer_count) + "\n"
            f.write(s)
        f.close()

    # overhead (data hops) per strategy
    for strategy in strategies:
        filename = './Results/overhead_' + str(strategy) 
        f = open(filename, 'w')
        f.write('# Overhead (in data hop count)\n')
        f.write('#\n')
        f.write('# Zipf Overhead\n')
        for alpha in alphas:
            data_hop_count = searchDictMultipleCat(lst, ['strategy', 'workload'], {'name' : strategy, 'alpha' : alpha}, 2, 'VIDEO_PLAYBACK', 'OVERHEAD')
            s = str(alpha) + "\t" + str(data_hop_count) + "\n"
            f.write(s)
        f.close()
    # Local Hits
    for strategy in strategies:
        filename = './Results/hits_' + str(strategy)
        f = open(filename, 'w')
        f.write('# Overhead (in data hop count)\n')
        f.write('#\n')
        f.write('# Zipf Hit_Ratio\n')
        for alpha in alphas:
            hit_ratio = searchDictMultipleCat(lst, ['strategy', 'workload'], {'name' : strategy, 'alpha' : alpha}, 2, 'VIDEO_PLAYBACK', 'CACHE_HIT_RATIO')
            s = str(alpha) + "\t" + str(hit_ratio) + "\n"
            f.write(s)
        f.close()

def print_caching_results(lst):

    strategies = ['NDN', 'NDN_MEASUREMENT', 'IPFS']
    n_chunks = 1000
    VIDEO_PLAYBACK_RATE = 10

    #buffer_sizes = [int(VIDEO_PLAYBACK_RATE), int(1.5*VIDEO_PLAYBACK_RATE), int(2*VIDEO_PLAYBACK_RATE), int(2.5*VIDEO_PLAYBACK_RATE), int(3.0*VIDEO_PLAYBACK_RATE)]
    #buffer_sizes = [int(VIDEO_PLAYBACK_RATE)] #, int(1.5*VIDEO_PLAYBACK_RATE), int(2*VIDEO_PLAYBACK_RATE), int(2.5*VIDEO_PLAYBACK_RATE), int(3.0*VIDEO_PLAYBACK_RATE)]
    N_CONTENTS = 100
    N_CHUNKS = 200 #1000
    TOTAL_CHUNKS = N_CHUNKS*N_CONTENTS
    cache_sizes = [TOTAL_CHUNKS*0.001, TOTAL_CHUNKS*0.005, TOTAL_CHUNKS*0.01, TOTAL_CHUNKS*0.05]
    ...
    for strategy in strategies:
        ...
        f.write('# Number of playback interruptions.\n')
        f.write('#\n')
        f.write('# Cache_Size Number_of_Interruptions\n')
        for cache_size in cache_sizes:
            buffer_count = searchDictMultipleCat(lst, ['strategy', 'cache_placement'], {'name' : strategy, 'network_cache_size' : cache_size}, 2, 'VIDEO_PLAYBACK', 'BUFFER_RATIO')
            s = str(cache_size) + "\t" + str(buffer_count) + "\n"
            f.write(s)
        f.close()
    ...
    for strategy in strategies:
        ...
        f.write('# Overhead (in data hop count)\n')
        f.write('#\n')
        f.write('# Buffer_Size Overhead\n')
        for cache_size in cache_sizes:
            data_hop_count = searchDictMultipleCat(lst, ['strategy', 'cache_placement'], {'name' : strategy, 'network_cache_size' : cache_size}, 2, 'VIDEO_PLAYBACK', 'OVERHEAD')
            s = str(cache_size) + "\t" + str(data_hop_count) + "\n"
            f.write(s)
        f.close()
    # Local Hits
    for strategy in strategies:
        ...
        f.write('# Overhead (in data hop count)\n')
        f.write('#\n')
        f.write('# Buffer_Size Hit_Ratio\n')
        for cache_size in cache_sizes:
            hit_ratio = searchDictMultipleCat(lst, ['strategy', 'cache_placement'], {'name' : strategy, 'network_cache_size' : cache_size}, 2, 'VIDEO_PLAYBACK', 'CACHE_HIT_RATIO')
            s = str(cache_size) + "\t" + str(hit_ratio) + "\n"
            f.write(s)
        f.close()

    # buffering events per content
    for strategy in strategies:
        for cache_size in cache_sizes:
            filename = './Results/per_content_buffering_' + str(strategy) + '_' + str(cache_size)
            f = open(filename, 'w')
            f.write('# Per content buffering events \n')
            f.write('#\n')
            f.write('# Buffer_Size Hit_Ratio\n')
            per_content_buffering_events = searchDictMultipleCat(lst, ['strategy', 'cache_placement'], {'name' : strategy, 'network_cache_size' : cache_size}, 2, 'VIDEO_PLAYBACK', 'BUFFERING_PER_CONTENT')
            for content_buffering in per_content_buffering_events:
                s = str(content_buffering[0][0]) + "\t" + str(content_buffering[1]) + "\n"
                f.write(s)
            f.close()



def print_buffering_results(lst):
    ...
    for strategy in strategies:
        ...
        f.write('# Number of playback interruptions.\n')
        f.write('#\n')
        f.write('# Buffer_Size Number_of_Interruptions\n')
        for buffer_size in buffer_sizes:
            buffer_count = searchDictMultipleCat(lst, ['strategy'], {'name' : strategy, 'wndw_size' : buffer_size}, 2, 'VIDEO_PLAYBACK', 'BUFFER_RATIO')
            s = str(buffer_size) + "\t" + str(buffer_count) + "\n"
            f.write(s)
        f.close()
    ...
    for strategy in strategies:
        ...
        f.write('# Buffer_Size Overhead\n')
        for buffer_size in buffer_sizes:
            data_hop_count = searchDictMultipleCat(lst, ['strategy'], {'name' : strategy, 'wndw_size' : buffer_size}, 2, 'VIDEO_PLAYBACK', 'OVERHEAD')
            s = str(buffer_size) + "\t" + str(data_hop_count) + "\n"
            f.write(s)
        f.close()
    # Local Hits
    for strategy in strategies:
        ...
        f.write('# Buffer_Size Hit_Ratio\n')
        for buffer_size in buffer_sizes:
            hit_ratio = searchDictMultipleCat(lst, ['strategy'], {'name' : strategy, 'wndw_size' : buffer_size}, 2, 'VIDEO_PLAYBACK', 'CACHE_HIT_RATIO')
            s = str(buffer_size) + "\t" + str(hit_ratio) + "\n"
            f.write(s)
        f.close()
    ...
    for strategy in strategies:
        for buffer_size in buffer_sizes:
            filename = './Results/per_content_buffering_' + str(strategy) + '_' + str(buffer_size)
            f = open(filename, 'w')
            f.write('# Per content buffering events \n')
            f.write('#\n')
            f.write('# Buffer_Size Hit_Ratio\n')
            per_content_buffering_events = searchDictMultipleCat(lst, ['strategy'], {'name' : strategy, 'wndw_size' : buffer_size}, 2, 'VIDEO_PLAYBACK', 'BUFFERING_PER_CONTENT')
            for content_buffering in per_content_buffering_events:
                s = str(content_buffering[0][0]) + "\t" + str(content_buffering[1]) + "\n"
                f.write(s)
            f.close()
...
def run(config, results, plotdir):
    ...
    print_buffering_results(lst)
    #print_caching_results(lst)
    #print_zipf_results(lst)
