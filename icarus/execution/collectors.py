...
class DataCollector(object):
    ...
    def report_buffering(self, content, flow_id):
        """ Reports buffering of a video playback
        """
...
class CollectorProxy(DataCollector):
    ...
    EVENTS = ('start_session', 'end_session', 'cache_hit', 'cache_miss', 'server_hit',
              'request_hop', 'content_hop', 'results', 'replacement_interval_over', 'execute_service'              ,'reassign_vm', 'report_buffering')
    ...
    @inheritdoc(DataCollector)
    def report_buffering(self, content, flow_id):
        for c in self.collectors['report_buffering']:
            c.report_buffering(content, flow_id)
...
@register_data_collector('VIDEO_PLAYBACK')
class VideoPlaybackCollector(DataCollector):
    """Collector keeps track of: 
    i) number of interruptions to the video playback, i.e. 
    segment to be consumed at a receiver has not arrived.
    ii) cache hits at the receivers.
    """
    ...
    def __init__(self, view):
        ...
        self.view = view
        self.sess_count = 0
        self.router_cache_hits = 0
        self.producer_cache_hits = 0
        self.serv_hits = 0
        self.n_interruptions_per_flow = {}
        self.n_interruptions_per_content = {}
        self.n_interruptions = 0
        self.req_path_len = 0
        self.cont_path_len = 0
        self.topo = self.view.topology()
        self.receivers = self.topo.graph['receivers']
        self.n_requests = 0

    @inheritdoc(DataCollector)
    def start_session(self, timestamp, receiver, content, flow_id, n_chunks):
        self.sess_count += 1
        self.n_interruptions_per_flow[flow_id] = 0
        self.n_chunks = n_chunks

    @inheritdoc(DataCollector)
    def report_buffering(self, content, flow_id):
        self.n_interruptions += 1
        self.n_interruptions_per_flow[flow_id] += 1
        
        video_content = int(content/self.n_chunks)
        if video_content not in self.n_interruptions_per_content.keys():
            self.n_interruptions_per_content[video_content] = 1
        else:
            self.n_interruptions_per_content[video_content] += 1

    @inheritdoc(DataCollector)
    def cache_hit(self, node):
        if self.topo.graph['receivers']:
            self.producer_cache_hits += 1
        elif self.topo.graph['routers']:
    ...
    @inheritdoc(DataCollector)
    def results(self):
        # number of hits (from receivers) per segment
        hit_ratio = (1.0*self.producer_cache_hits) / self.n_requests
        # average number of buffering per number of video sessions
        interruption_ratio = (1.0*self.n_interruptions) / self.sess_count 
        results = Tree(**{'CACHE_HIT_RATIO': hit_ratio})
        results['BUFFER_RATIO'] = interruption_ratio
        results['OVERHEAD'] = 1.0*self.cont_path_len/self.n_requests
        results['BUFFERING_PER_CONTENT'] = self.n_interruptions_per_content
        router_hit_ratio = (1.0*self.router_cache_hits)/self.n_requests 
        results['ROUTER_HIT_RATIO'] = router_hit_ratio
