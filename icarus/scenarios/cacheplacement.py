...
def edge_cache_placement(topology, cache_budget, network_cache_size=1, **kwargs):
    """Places cache budget uniformly across cache nodes.
    ...
    cache_size = cache_budget #iround(cache_budget / len(icr_candidates))
    print("HERE")
    for v in cache_nodes:
        if v in topology.graph['receivers']:
            topology.node[v]['stack'][1]['cache_size'] = cache_size
            #print ("Adding cache to node: " + str(v) + " of size: " + str(cache_size))
        else:
            topology.node[v]['stack'][1]['cache_size'] = network_cache_size
            #print ("Adding cache to node: " + str(v) + " of size: " + str(network_cache_size))
