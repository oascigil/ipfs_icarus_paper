...
INTERNAL_LINK_DELAY = 0.004
EXTERNAL_LINK_DELAY = 0.350
...
def topology_tiscali(nrecv_per_edge, **kwargs):
    ...
    sources = [random.choice(onedeg)]  
    
    # 80 edge routers 
    edge_routers = [v for v in onedeg if v not in sources]
    
    mid_routers = []
    total_degree = 0
    for v in edge_routers:
        neighbors = list(topology.edge[v].keys())
        total_degree += len(neighbors)
        for v in neighbors:
            if v not in mid_routers:
                mid_routers.append(v)
    average_degree = 1.0*total_degree/len(edge_routers)
    #print ("Average degree of edge routers: " + str(average_degree))
    
    core_routers2 = []
    total_degree = 0
    for v in mid_routers:
        neighbors = list(topology.edge[v].keys())
        total_degree += len(neighbors)
        for v in neighbors:
            if v not in core_routers2 and v not in edge_routers and v not in mid_routers:
                core_routers2.append(v)
    average_degree = 1.0*total_degree/len(mid_routers)
    #print ("Average degree of tier-4 routers: " + str(average_degree))

    core_routers = []
    total_degree = 0
    for v in core_routers2:
        neighbors = list(topology.edge[v].keys())
        total_degree += len(neighbors)
        for v in neighbors:
            if v not in core_routers and v not in edge_routers and v not in mid_routers and v not in core_routers2:
                core_routers.append(v)
    average_degree = 1.0*total_degree/len(core_routers2)
    #print ("Average degree of tier-3 routers: " + str(average_degree))
    
    top_tier_routers = []
    total_degree = 0
    for v in core_routers:
        neighbors = list(topology.edge[v].keys())
        total_degree += len(neighbors)
        for v in neighbors:
            if v not in core_routers and v not in edge_routers and v not in mid_routers and v not in core_routers2 and v not in top_tier_routers:
                top_tier_routers.append(v)
    average_degree = 1.0*total_degree/len(core_routers)
    #print ("Average degree of tier-2 routers: " + str(average_degree))
    
    total_degree = 0
    for v in top_tier_routers:
        neighbors = list(topology.edge[v].keys())
        total_degree += len(neighbors)
    average_degree = 1.0*total_degree/len(top_tier_routers)
    #print ("Average degree of tier-1 routers: " + str(average_degree))
    
    # set weights and delays on all links
    fnss.set_weights_constant(topology, 1.0)
    fnss.set_delays_constant(topology, INTERNAL_LINK_DELAY, 'ms')

    # Attach artificial receiver nodes to edge_routers
    receivers = ['rec_%d' % i for i in range(len(edge_routers) * nrecv_per_edge)]
    print ("There are " + str(len(receivers)) + " receivers in the topology")
    for i in range(len(receivers)):
        topology.add_edge(receivers[i], edge_routers[i%nrecv_per_edge], delay=0.001, type='internal')
    
    icr_candidates = [v for v in topology.nodes() if v not in sources]  # 36 nodes
    rsn_candidates = [v for v in edge_routers]  # 36 nodes
    
    # we set router stacks because some strategies will fail if no stacks
    # are deployed
    routers = [v for v in topology.nodes() if v not in sources + receivers]
    
    rest_routers = [v for v in routers if v not in core_routers2 + core_routers + edge_routers + mid_routers + top_tier_routers]
    #print ("There are remaining " + str(len(rest_routers)) + " that are not in edge or core routers")
    top_tier_routers = top_tier_routers + rest_routers

    # Deploy stacks
    topology.graph['icr_candidates'] = set(icr_candidates)
    topology.graph['rsn_candidates'] = set(rsn_candidates)
    topology.graph['receivers'] = receivers
    topology.graph['sources'] = sources
    topology.graph['routers'] = routers
    topology.graph['edge_routers'] = edge_routers
    topology.graph['core_routers'] = top_tier_routers
    topology.graph['receiver_access_delay'] = 0.001
    ...
    #print "There are " + repr(len(mid_routers)) + " tier-4 routers: " + repr(mid_routers)
    #print "There are " + repr(len(core_routers2)) + " tier-3 routers: " + repr(core_routers2)
    #print "There are " + repr(len(core_routers)) + " tier-2 routers: " + repr(core_routers)
    #print "There are " + repr(len(top_tier_routers)) + " tier-1 routers: " + repr(top_tier_routers)
