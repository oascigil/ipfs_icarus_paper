...
def uniform_content_placement(topology, contents, n_chunks, seed=None):
   ...
   contents : iterable
    ...
    for c in range(len(contents)*n_chunks):
        if c%n_chunks == 0:
            server = random.choice(source_nodes)
            content_placement[server].add(c)
        else:
            content_placement[server].add(c)
