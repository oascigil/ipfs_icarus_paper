...
@register_rsn_placement('EDGE')
def uniform_edge_rsn_placement(topology, rsn_budget, **kwargs):
    """Place rsn caches at the edge routers only.
    ...
    topology : Topology
        The topology object
    rsn_budget : int
        The per-node rsn cache budget (in number of entries across the network)
    """
    
    rsn_candidates = topology.graph['rsn_candidates']
    # cutoff node must be at least one otherwise, if spread is too low, no
    # nodes would be selected
    rsn_size = rsn_budget
    print ("Per edge-router RSN size is set to: " + str(rsn_budget))

    if rsn_size == 0:
        return
    for v in rsn_candidates:
...
@register_joint_cache_rsn_placement('CACHE_ALL_RSN_EDGE')
def cache_all_rsn_edge_placement(topology, cache_budget, rsn_budget,
                                 rsn_spread=0.5, metric_dict=None, **kwargs):
    """Jointly assign caches and RSN tables with caches in all candidate nodes
    and RSN tables in nodes with top centralities.
    ...
    uniform_consolidated_cache_placement(topology, cache_budget,
                                         spread=1.0,
                                         metric_dict=metric_dict, target='top')
    uniform_edge_rsn_placement(topology, rsn_budget)
