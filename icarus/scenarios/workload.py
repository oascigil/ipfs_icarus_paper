...
class StationaryWorkload(object):
    ...
    """
    def __init__(self, topology, n_contents, n_chunks, alpha, beta=0, rate=1.0,
        ...
        self.zipf = TruncatedZipfDist(alpha, n_contents-1, seed)
        ...
        self.available_receivers = copy.copy(self.receivers)
        self.seed = seed
        self.first = True
    ...
    def __iter__(self):
        ...
        while req_counter < self.n_warmup + self.n_measured or len(self.model.eventQ) > 0:
            ...
            while eventObj is not None and eventObj.time < t_event:
                ...
                event = {'receiver' : eventObj.receiver, 'content': eventObj.content, 'log' : log, 'node' : eventObj.node, 'flow_id' : eventObj.flow_id, 'deadline' : eventObj.deadline, 'rtt_delay' : eventObj.rtt_delay, 'status' : eventObj.status, 'task' : eventObj.task, 'prev_hop': eventObj.prev_hop, 'destination' : eventObj.destination}
                if  (eventObj.status == SESSION_COMPLETE):
                    #print ("A receiver: " + str(eventObj.receiver) + " completed session - detected in workload.")
                    #print ("Last consume event - @content: " + str(eventObj.content) + " @node: " + str(eventObj.node) + " @status: " + str(eventObj.status))
                    self.available_receivers.append(eventObj.receiver)
                    if no_receivers is True:
                        no_receivers = False
            ...
            if len(self.available_receivers) == 0:
                #if no_receivers is False:
                #    print ("No available receiver in workload to choose. Num. of active events: " + str(len(self.model.eventQ)))
                no_receivers = True
                continue

            receiver = random.choice(self.available_receivers)
            self.available_receivers.remove(receiver)

            node = receiver
            content = int(self.zipf.rv()) * self.n_chunks
            log = (req_counter >= self.n_warmup)
            flow_id += 1
