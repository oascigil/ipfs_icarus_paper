...
class Task(object):
    ...
    def __init__(self, time, taskType, deadline, rtt_delay, node, service, service_time, flow_id, receiver, arrivalTime=None, completionTime=None, content=None, prev_hop=None):
        ### Type of the task #
        ...
        self.nextTask = []
        # content id for content request task
        self.content = content
        # prev_hop where the task request arrived from
    ...
    def print_task(self):
        print ("---Task.service: " + str(self.service))
        print ("\tTask.time: " + str(self.time))
        print ("\tTask.type: " + str(self.taskType))
        print ("\tTask.deadline: " + str(self.expiry))
        print ("\tTask.rtt_delay: " + str(self.rtt_delay))
        print ("\tTask.exec_time: " + str(self.exec_time))
        print ("\tTask.node: " + str(self.node))
        print ("\tTask.flow_id: " + str(self.flow_id))
        print ("\tTask.receiver: " + str(self.receiver))
        print ("\tTask.completionTime: " + str(self.completionTime))
        ...
        if self.arrivalTime is not None:
            print ("\tTask.arrivalTime: " + str(self.arrivalTime))
        if len(self.nextTask) > 0:
            print ("\tTask.nextTask: ")
            for atask in self.nextTask:
                atask.print_task()
        if self.content is not None:
            print("\tTask.content: " + str(self.content))
        print ("\n")
...
class Scheduler(object):
    ...
    def schedule_without_vm_allocation(self, time, numVMs, numStartingVMs, fail_if_no_vm = True, debug=True):
        """
        Return the next task to be executed, if there is any.
        """
        core_indx = self.get_available_core(time, False)
        
        if time == float('inf') or time == None:
                ...
                if number_of_vms > 0: 
                    ...
                    if available_vms > 0:
                        ...
                        if aTask.taskType == Task.TASK_TYPE_VM_START:
                            ...
                            if numStartingVMs[newService] > 0:
                                numStartingVMs[newService] -= 1
                                numVMs[newService] += 1
                            else:
                                numVMs[oldService] -= 1
                                numVMs[newService] += 1
                            aTask.service = aTask.serviceToInstantiate 
                            self.assign_task_to_cpu_core(core_indx, aTask, time)
                            aTask.service = oldService
...
class ComputationSpot(object):
    ...
    def compute_completion_times(self, time, fail_if_no_vm = True, debug=False):
        ...
        """
        for aTask in self.scheduler._taskQueue + self.scheduler.upcomingTaskQueue:
            service = aTask.service 
            numVMs[service] = len(self.scheduler.idleVMs[service]) + len(self.scheduler.busyVMs[service]) 
            numStartingVMs[service] = len(self.scheduler.startingVMs[service])
            if aTask.taskType == Task.TASK_TYPE_VM_START and aTask.serviceToInstantiate is not None:
        ...
        #for utask in self.schedulerCopy.upcomingTaskQueue:
        #    if utask.flow_id == 1964:
        #        debug = True
        #        break
        ...
        while len(self.schedulerCopy._taskQueue) > 0 or len(self.schedulerCopy.upcomingTaskQueue) > 0:
            taskToExecute = self.schedulerCopy.schedule_without_vm_allocation(time, numVMs, numStartingVMs, fail_if_no_vm, debug)
    ...
    def admit_content_request_task(self, service, content, time, flow_id, deadline, receiver, rtt_delay, controller, debug=False):
        """
        Admit a content request at a receiver with a constant upload rate.
        """
        #serviceTime = self.services[service].service_time
        #aTask = Task(time, Task.TASK_TYPE_SERVICE, deadline, rtt_delay, self.node, service, serviceTime, flow_id, receiver, None, None, content)
        #self.scheduler.addToTaskQueue(aTask, time)
        self.running_requests[service] += 1
        # Run the next task (if there is any)
        newTask = self.scheduler.schedule(time) 
        if newTask is not None:
            if debug:
                print ("A task complete event is scheduled. @time: " + str(newTask.completionTime) + " @receiver: "  + str(newTask.receiver) + " @content: " + str(newTask.content) + " @node: " + str(self.node))
            controller.add_event(newTask.completionTime, newTask.receiver, newTask.content, self.node, newTask.flow_id, newTask.expiry, newTask.rtt_delay, TASK_COMPLETE, newTask) 
        """
        else:
            print("admit_content_request_task() did not schedule any task(). ")
            This should not happen in admit_content_request_task(): receiver: " + str(self.node) + " has nothing in its upcoming task queue")
            print ("@time: " + str(time) + " @content: " + str(content) + " @receiver: " + str(receiver))
            print ("Upcoming task queue: " )
            for task in self.scheduler.upcomingTaskQueue:
                task.print_task()
            print ("Task queue: ")
            for task in self.scheduler._taskQueue:
                task.print_task()
            #raise ValueError("This should not happen in admit_content_request_task(): receiver: " + str(self.node) + " has nothing in its upcoming task queue")
        """
        return [True, SUCCESS]
